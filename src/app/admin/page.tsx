...
export default function AdminDashboard() {
  ...
  const [selectedInventory, setSelectedInventory] = useState<{[key: string]: { selected: boolean, quantity: number }}>({});
  ...
  const handleCreateDistroWithStore = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1.5 Prepare Inventory Assignment (Log for now or loop updateStock)
      const assignments = Object.entries(selectedInventory)
        .filter(([_, data]) => data.selected && data.quantity > 0)
        .map(([id, data]) => ({ productId: id, qty: data.quantity }));
      
      console.log("Asignando inventario inicial:", assignments);
      // In a real scenario, we would link these to the new store.
      ...
      const link = `${window.location.origin}/auth/invitation?token=${token}&email=${encodeURIComponent(newDistro.email)}`;
  ...
  const handleCreateAdminOnly = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      const resp = await authService.createAdmin({
        ...newAdmin,
        password: newAdmin.password || "Ozono2026!",
        role: 'ADMIN'
      });
      const token = resp?.id || "admin-token-" + Math.random().toString(36).substr(2, 5);
      const link = `${window.location.origin}/auth/invitation?token=${token}&email=${encodeURIComponent(newAdmin.email)}`;
  ...
  if (isLoading || !user || user.rol !== 'ROOT_ADMIN') {
    return (
    ...
    <div className="flex h-screen bg-surface relative overflow-hidden font-sans text-on-surface">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-surface-container-lowest border-r border-outline-variant/10 flex flex-col shadow-2xl z-50 transition-transform duration-500 ease-spring lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 p-6 space-y-3">
          ...
          <div className="pt-4 pb-2">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40 px-5 mb-2">Creación</p>
            {user.rol === 'ROOT_ADMIN' && (
              <button onClick={() => { setActiveTab("new-admin"); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${activeTab === "new-admin" ? "bg-primary text-on-primary shadow-xl shadow-primary/20" : "text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface"}`}>
                <span className="material-symbols-outlined text-[20px]">key</span>
                <span className="text-sm font-black tracking-tight">Nuevo Admin</span>
              </button>
            )}
            <button onClick={() => { setActiveTab("new-distro"); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${activeTab === "new-distro" ? "bg-secondary text-on-secondary shadow-xl shadow-secondary/20" : "text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface"}`}>
              <span className="material-symbols-outlined text-[20px]">person_add</span>
              <span className="text-sm font-black tracking-tight">Distribuidor</span>
            </button>
          </div>
        </nav>
      ...
      <main className="flex-1 overflow-y-auto p-6 md:p-10 lg:p-14 space-y-12 bg-surface">
        <header className="flex flex-col gap-8 relative z-10">
          ...
          {activeTab === "new-admin" && (
            <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-500">
              <h2 className="font-headline text-5xl md:text-6xl font-black text-on-surface tracking-tighter leading-[0.9]">Nuevo Admin.</h2>
              <p className="text-on-surface-variant text-lg font-medium opacity-60">Configuración de un nuevo administrador del sistema.</p>
            </div>
          )}
          {activeTab === "new-distro" && (
            <div className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-500">
              <h2 className="font-headline text-5xl md:text-6xl font-black text-on-surface tracking-tighter leading-[0.9]">Nuevo Distribuidor.</h2>
              <p className="text-on-surface-variant text-lg font-medium opacity-60">Alta de distribuidor con asignación de tienda e inventario inicial.</p>
            </div>
          )}
        </header>
        ...
        {activeTab === "new-admin" && (
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="bg-surface-container-lowest rounded-[48px] shadow-2xl overflow-hidden border border-outline-variant/10">
              <div className="p-10 border-b border-outline-variant/10 bg-primary/[0.02]">
                <h3 className="font-headline text-2xl font-black text-on-surface tracking-tighter uppercase">Registro de Administrador</h3>
              </div>
              <form onSubmit={handleCreateAdminOnly} className="p-10 space-y-8">
                <div className="p-6 bg-primary/5 rounded-3xl border border-primary/10 flex gap-6 items-center">
                  <span className="material-symbols-outlined text-primary text-3xl">info</span>
                  <p className="text-xs font-bold text-primary opacity-80 leading-relaxed uppercase tracking-tight">
                    Este perfil tiene facultades para supervisar la red global y gestionar otros administradores. No requiere una tienda física propia.
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-60 px-2">Nombre Completo</label>
                    <input required type="text" value={newAdmin.firstName} onChange={e => setNewAdmin({...newAdmin, firstName: e.target.value})} className="w-full bg-surface-container border border-outline-variant/30 rounded-2xl py-5 px-6 text-sm font-bold focus:ring-4 focus:ring-primary/10 outline-none transition-all" placeholder="Ej: Juan" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-60 px-2">Apellido</label>
                    <input required type="text" value={newAdmin.lastName} onChange={e => setNewAdmin({...newAdmin, lastName: e.target.value})} className="w-full bg-surface-container border border-outline-variant/30 rounded-2xl py-5 px-6 text-sm font-bold focus:ring-4 focus:ring-primary/10 outline-none transition-all" placeholder="Ej: Perez" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-60 px-2">Email Profesional</label>
                    <input required type="email" value={newAdmin.email} onChange={e => setNewAdmin({...newAdmin, email: e.target.value})} className="w-full bg-surface-container border border-outline-variant/30 rounded-2xl py-5 px-6 text-sm font-bold focus:ring-4 focus:ring-primary/10 outline-none transition-all" placeholder="admin@superozono.com" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant opacity-60 px-2">Contraseña Temporal</label>
                    <input required type="text" value={newAdmin.password} onChange={e => setNewAdmin({...newAdmin, password: e.target.value})} className="w-full bg-surface-container border border-outline-variant/30 rounded-2xl py-5 px-6 text-sm font-bold focus:ring-4 focus:ring-primary/10 outline-none transition-all" placeholder="Ozono2026!" />
                  </div>
                </div>
                <div className="pt-8 block">
                  <button disabled={isCreating} type="submit" className="w-full py-6 bg-on-surface text-surface rounded-[32px] font-black text-sm tracking-[0.2em] uppercase hover:scale-[1.02] transition-all active:scale-95 shadow-2xl disabled:opacity-50">
                    {isCreating ? 'Procesando...' : 'CREAR PERFIL ADMINISTRATIVO'}
                  </button>
                </div>
              </form>
            </div>
        ...
        {/* NEW DISTRIBUTOR FORM VIEW + INVENTORY */}
        {activeTab === "new-distro" && (
          <div className="max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
            <div className="bg-surface-container-lowest rounded-[48px] shadow-2xl overflow-hidden border border-outline-variant/10">
              <div className="p-10 border-b border-outline-variant/10 bg-secondary/[0.02]">
                <h3 className="font-headline text-2xl font-black text-on-surface tracking-tighter uppercase">Alta Integral de Distribuidor</h3>
              </div>
              <form onSubmit={handleCreateDistroWithStore} className="p-10 space-y-12">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                  ...
                  <div className="space-y-8">
                    <div className="flex items-center gap-4">
                      ...
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-on-surface-variant">Datos Personales</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Nombre</label>
                        <input required type="text" value={newDistro.firstName} onChange={e => setNewDistro({...newDistro, firstName: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-primary outline-none transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Apellido</label>
                        <input required type="text" value={newDistro.lastName} onChange={e => setNewDistro({...newDistro, lastName: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-primary outline-none transition-all" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Email</label>
                      <input required type="email" value={newDistro.email} onChange={e => setNewDistro({...newDistro, email: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-primary outline-none transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Password Provisional</label>
                      <input required type="text" value={newDistro.password} onChange={e => setNewDistro({...newDistro, password: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-primary outline-none transition-all" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div className="flex items-center gap-4">
                      ...
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-on-surface-variant">Configuración de Tienda</h4>
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Nombre de la Tienda</label>
                      <input required type="text" value={newDistro.storeName} onChange={e => setNewDistro({...newDistro, storeName: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-secondary outline-none transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Subdominio</label>
                      <div className="relative">
                        <input required type="text" value={newDistro.subdomain} onChange={e => setNewDistro({...newDistro, subdomain: e.target.value.toLowerCase().replace(/\s+/g, '-')})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-secondary outline-none transition-all pr-24" />
                        <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black opacity-30">.superozono</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Dirección</label>
                        <input required type="text" value={newDistro.address} onChange={e => setNewDistro({...newDistro, address: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-secondary outline-none transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-40 px-2">Teléfono</label>
                        <input required type="tel" value={newDistro.phone} onChange={e => setNewDistro({...newDistro, phone: e.target.value})} className="w-full bg-surface-container/50 border border-outline-variant/20 rounded-2xl py-4 px-5 text-sm font-bold focus:border-secondary outline-none transition-all" />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8 pt-8 border-t border-outline-variant/10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-[0.2em] text-on-surface-variant">Inventario Inicial Asignado</h4>
                      <p className="text-[10px] font-bold text-on-surface-variant/40 mt-1 uppercase">Define los productos con los que iniciará esta sucursal</p>
                    </div>
                  </div>
                  
                  <div className="bg-surface-container/20 rounded-3xl border border-outline-variant/10 overflow-hidden">
                    <div className="overflow-x-auto scrollbar-hide">
                      <table className="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                          <tr className="bg-surface-container-low/50 text-on-surface-variant/60 uppercase text-[9px] font-black tracking-widest">
                            <th className="px-8 py-5">Producto</th>
                            <th className="px-8 py-5">Precio Sugerido</th>
                            <th className="px-8 py-5">Cantidad Inicial</th>
                            <th className="px-8 py-5 text-right">Añadir</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-outline-variant/5">
                          {allProducts.map(product => (
                            <tr key={product.id} className="hover:bg-primary/5 transition-colors">
                              <td className="px-8 py-4">
                                <p className="font-black text-on-surface text-sm">{product.name}</p>
                                <p className="text-[10px] text-on-surface-variant font-bold opacity-40">REF: {product.sku || 'N/A'}</p>
                              </td>
                              <td className="px-8 py-4">
                                <p className="text-sm font-bold text-on-surface-variant">${product.price}</p>
                              </td>
                              <td className="px-8 py-4">
                                 <input 
                                   type="number" 
                                   min="0" 
                                   value={selectedInventory[product.id]?.quantity || 0}
                                   onChange={(e) => setSelectedInventory(prev => ({
                                     ...prev,
                                     [product.id]: {
                                   ...
                                   className="w-24 bg-surface-container border border-outline-variant/20 rounded-xl py-2 px-3 text-sm font-bold" 
                                 />
                              </td>
                              <td className="px-8 py-4 text-right">
                                <div className="flex justify-end">
                                  <input 
                                    type="checkbox" 
                                    checked={selectedInventory[product.id]?.selected || false}
                                    onChange={(e) => setSelectedInventory(prev => ({
                                      ...prev,
                                      [product.id]: {
                ...
                <div className="pt-10 flex justify-end">
                   <button disabled={isCreating} type="submit" className="px-16 py-6 bg-on-surface text-surface rounded-[32px] font-black text-sm tracking-[0.2em] uppercase hover:scale-[1.02] transition-all active:scale-95 shadow-2xl disabled:opacity-50">
                    {isCreating ? 'Sincronizando...' : 'FINALIZAR Y REGISTRAR'}
                  </button>
                </div>
              </form>
            </div>
      ...
      {/* Modal sections removed as they are now full pages */}
